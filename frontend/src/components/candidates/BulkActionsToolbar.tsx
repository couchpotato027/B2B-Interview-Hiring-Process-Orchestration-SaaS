...
export default function BulkActionsToolbar({ 
    ...
    return (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-8 duration-300">
            <div className="bg-[#0a0f1a] text-white rounded-2xl shadow-2xl px-6 py-4 flex items-center gap-6 border border-white/10 backdrop-blur-xl">
                ...
                {isProcessing ? (
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="relative">
                            ...
                            {showStageMenu && (
                                <div className="absolute bottom-full mb-2 left-0 w-48 bg-white text-slate-900 rounded-xl shadow-2xl ring-1 ring-black/5 p-1">
                                    {availableStages.map(stage => (
                                        <button 
                                            key={stage.id}
                                            onClick={() => handleBulkAction('MOVE_STAGE', { newStageId: stage.id })}
                                            className="w-full text-left px-3 py-2 rounded-lg hover:bg-slate-50 text-sm font-medium transition-colors"
                                        >
                                            {stage.name}
                                        </button>
                                    ))}
                        ...
                        <div className="relative">
                            ...
                            {showUserMenu && (
                                <div className="absolute bottom-full mb-2 left-0 w-48 bg-white text-slate-900 rounded-xl shadow-2xl ring-1 ring-black/5 p-1 max-h-60 overflow-y-auto">
                                    {users.map(user => (
                                        <button 
                                            key={user.id}
